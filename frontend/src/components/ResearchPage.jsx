      ...
      <section className="research-section" id="ensemble">
        <div className="section-inner">
          ...
          <div className="ensemble-diagram">
            <div className="flow-node">입력 X-ray</div>
            <div className="flow-arrow">↓</div>
            <div className="flow-row">
              {ENSEMBLE_MODELS.map((m) => (
                <div className="flow-model" key={m.name}>
                  <span className="flow-weight">×{m.weight}</span>
                  {m.name}
                </div>
              ))}
            </div>
            <div className="flow-arrow">가중 합 (2:1:2) / 5 → MAX 집계 → 임계 0.652</div>
            <div className="flow-node result">골절 / 정상</div>
          </div>
          <div className="ensemble-cards">
            {ENSEMBLE_MODELS.map((m) => (
              <div className="ensemble-card" key={m.name}>
                <div className="ensemble-card-head">
                  <h3>{m.name}</h3>
